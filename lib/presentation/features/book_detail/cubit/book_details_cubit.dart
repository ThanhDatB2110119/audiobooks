...
class BookDetailsCubit extends Cubit<BookDetailsState> {
  ...
  Future<void> fetchBookDetails(String id) async {
    ...
    bookResult.fold(
      ...
      (book) {
        isSavedResult.fold(
          (failure) => emit(BookDetailsLoaded(book, isSaved: false)), // Mặc định là chưa lưu nếu lỗi
  ...
  Future<void> toggleSaveStatus() async {
    ...
    if (isCurrentlySaved) { // Nếu UI đang là "đã lưu", nghĩa là hành động là "bỏ lưu"
      result = await _removeBookFromLibraryUsecase(bookId.toString());
    } else { // Ngược lại, hành động là "lưu"
    ...
    result.fold(
      ...
      (_) => null, // Thành công, không cần làm gì
    );
  }
}
